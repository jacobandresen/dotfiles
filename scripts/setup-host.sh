...
#   • mu        — pins the model (MU_AGENT_MODEL, mirroring pi) and the KV-cache
#                 context (MU_NUM_CTX) the card can hold to ~/.zshrc.mu
#                 (machine-local, outside the repo). Only mu reads these.
...
# the per-host context lives in the untracked ~/.zshrc.mu (sourced by .zshrc
...
#
# Idempotent: re-run after a GPU change and it rewrites the profile — including
# ~/.zshrc.mu, which is regenerated on every host (it always pins MU_AGENT_MODEL).
set -euo pipefail
...
# ── 2. mu: per-host model + KV-cache context via ~/.zshrc.mu ──────────────────
# zshrc.mu pins the mu agent's model (MU_AGENT_MODEL) and, on a roomier card, its
# KV-cache context (MU_NUM_CTX). MU_AGENT_MODEL mirrors pi's defaultModel so mu and
# pi share one local model, and it's pinned on *every* host: without it mu
# auto-selects the first /v1/models entry, which can be a model too large to load.
# MU_NUM_CTX is written only when it exceeds mu's own 6000 default. This is the
# machine-local file, outside the repo — never tracked.
echo "── mu (~/.zshrc.mu) ─────────────────────────────────────────"
{
    echo "# ~/.zshrc.mu — generated by scripts/setup-host.sh. Do not edit;"
    echo "# re-run 'make setup-host' to regenerate. Machine-local, outside the repo."
    echo "# Host: $(uname -n) | GPU: $GPU_DESC | profile: $PROFILE"
    echo ""
    echo "# Same local model as pi (see ~/.pi/agent/settings.json defaultModel)."
    echo "export MU_AGENT_MODEL=$PI_DEFAULT_MODEL"
    [ "$NUM_CTX" != "6000" ] && echo "export MU_NUM_CTX=$NUM_CTX"
} > "$ZSHRC_MU"
if [ "$NUM_CTX" = "6000" ]; then
    echo "  ✓ Wrote $ZSHRC_MU (MU_AGENT_MODEL=$PI_DEFAULT_MODEL; MU_NUM_CTX uses mu's 6000 default)"
else
    echo "  ✓ Wrote $ZSHRC_MU (MU_AGENT_MODEL=$PI_DEFAULT_MODEL, MU_NUM_CTX=$NUM_CTX)"
fi
echo ""
